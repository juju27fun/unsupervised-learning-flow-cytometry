...
def _validate_checkpoint(
    ...
    *,
    ...
    if int(payload["epoch"]) != expected_epoch:
        raise ValueError(
            f"{policy} checkpoint must be frozen at epoch {expected_epoch}"
        )
...
def initialize_paired_models(
    ...
    *,
    ...
    device: torch.device,
    expected_checkpoint_epoch: int = 20,
    ...
    for policy, path in checkpoint_paths.items():
        ...
        _validate_checkpoint(
            ...
            seed=seed,
            config=config,
            expected_epoch=expected_checkpoint_epoch,
        )
...
def run_paired_finetuning(
    ...
    *,
    ...
    output_dir: Path,
    expected_checkpoint_epoch: int = 20,
    protocol: str = "bead-downstream-finetuning-v1",
) -> dict[str, Any]:
