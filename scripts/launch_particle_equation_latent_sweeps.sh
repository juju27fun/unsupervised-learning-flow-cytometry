...
# Published single-particle figure
# --------------------------------
# artifacts/unsupervised-learning-flow-cytometry/particle_equation_latent_sweeps/
# single_n1800_figure_based was produced by calling the CLI directly with
# --input-length 512, --signal-window-duration-ms 1.0,
# --single-sweep-source paper_table and --realistic-figure-based-sweeps.
# Those are NOT the defaults below (4096 / 2.048 ms / legacy), which serve the
# exploratory and yeast modes. Use `published-single` to reproduce the figure.
...
INPUT_LENGTH="${INPUT_LENGTH:-4096}"
...
# Rebuilt per run so a mode can flip a switch after this point.
build_extra_args() {
  extra_args=()
  if [[ "$SKIP_TSNE" == "1" ]]; then
...
case "$MODE" in
  ...
  single)
    ...
    ;;
  published-single)
    # Exact settings of the published figure, reproducing
    # particle_equation_latent_sweeps/single_n1800_figure_based.
    INPUT_LENGTH=512
    SIGNAL_WINDOW_DURATION_MS=1.0
    SINGLE_SWEEP_SOURCE=paper_table
    PHASE_PROFILE=table_mean
    REALISTIC_FIGURE_BASED_SWEEPS=1
    run_scenario single_particle single_n${N_PER_PANEL}_figure_based
    ;;
  two)
  ...
  all)
    ...
    ;;
  *)
    echo "Usage: $0 {smoke|single|published-single|two|yeast-proof|yeast-two|all}" >&2
    echo "published-single reproduces the published figure: input length 512, 1.0 ms window, paper_table sweeps, figure-based tau ranges." >&2
    echo "Every other mode keeps the exploratory defaults: input length 4096, 2.048 ms window, legacy sweeps." >&2
    echo "Optional env: N_PER_PANEL, INPUT_LENGTH, BATCH_SIZE, DEVICE, SKIP_TSNE, MODELS, BASE_OUTPUT_DIR, SINGLE_SWEEP_SOURCE, PHASE_PROFILE, SIGNAL_WINDOW_DURATION_MS, REALISTIC_FIGURE_BASED_SWEEPS, YEAST_RANGE_SUMMARY, YEAST_BUDDED_STYLE, YEAST_BACKGROUND_NOISE_SCALE, SIGNALS_ONLY, PYTHON" >&2
    exit 2
    ;;
esac
